...
export function SolutionMacInteractiveHero({
  doc,
  slug,
  initialMacFocus,
}: {
  ...
  const tabNav = (
    <nav className="mt-6" aria-label="Solution feature areas">
        ...
        {sections.map((sec, i) => {
          ...
          return (
            <button
              ...
              type="button"
              role="tab"
              id={`solution-mac-tab-${i}`}
              aria-selected={selected}
              aria-controls="solution-mac-panel"
              onClick={() => {
              ...
              className={`w-full flex items-center gap-2.5 rounded-[var(--radius-ui)] border px-3 py-2.5 text-left font-sans text-[13px] leading-snug transition-colors md:text-[14px] lg:py-3 lg:text-[15px] ${
                selected
            ...
            >
              <Icon className="h-4 w-4 shrink-0 opacity-70" strokeWidth={1.5} aria-hidden />
  ...
  const mac = (
    <div className="relative w-full min-w-0 max-w-[min(100%,38rem)] shrink-0 sm:max-w-[min(100%,44rem)] lg:max-w-[min(100%,50rem)] xl:max-w-[min(100%,58rem)]">
      ...
      >
        ...
        <div
          id="solution-mac-panel"
          role="tabpanel"
          aria-labelledby={`solution-mac-tab-${safeIndex}`}
        ...
        >
          <div className="mb-5 flex items-center justify-between border-b border-light-steel pb-3.5 md:mb-6 md:pb-4">
            <span className="flex items-center gap-2 font-sans text-[12px] font-semibold text-deep-graphite md:text-[13px]">
              <Icon className="h-4 w-4 text-amber-glow" strokeWidth={1.35} aria-hidden />
              {doc.heroTitle}
            </span>
  ...
  return (
    <section
      ...
      aria-label="Solution overview and interactive preview"
    >
